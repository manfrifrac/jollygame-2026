...
async function finalConsolidation() {
  console.log("🚀 Consolidamento Serie Gre (LOGICA DEFINITIVA)...");
  ...
  for (const series in groups) {
      ...
      // 1. Rinomina opzione "Title" in "Misura" sul Master se necessario
      const currentOption = master.options.find((o: any) => o.name === "Title");
      if (currentOption) {
          const optRes = await shopifyRequest(`
      ...
      // 2. Aggiorna il titolo del Master con il nome della Serie
      ...
      // 3. Raccogli e aggiungi varianti
      ...
      for (const other of others) {
          for (const v of other.variants.nodes) {
              ...
              let dim = dimMatch ? dimMatch[0] : "Altra Misura";
      ...
      const addRes = await shopifyRequest(`
      ...
      if (!addRes.data?.productVariantsBulkCreate?.userErrors?.length) {
          console.log(`   ✅ Varianti aggiunte con successo.`);
          // 4. Elimina gli altri
          for (const other of others) {
              await shopifyRequest(`mutation { productDelete(input: { id: "${other.id}" }) { deletedProductId } }`);
          }
      } else {
          console.error(`   ❌ Errore durante l'aggiunta varianti:`, addRes.data.productVariantsBulkCreate.userErrors);
      }
  }

  console.log("\n✅ Operazione di consolidamento Serie Gre completata.");
}
