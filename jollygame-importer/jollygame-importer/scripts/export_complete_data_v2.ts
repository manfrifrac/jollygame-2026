...
async function exportEverythingFixed() {
  console.log("📥 Avvio Esportazione MASSIVA (FIXED SCHEMA)...");
  ...
  while (hasNextPage) {
    ...
    if (!res.data?.products) {
        console.error("❌ Errore:", JSON.stringify(res.errors, null, 2));
        break;
    }
  ...
  console.log("\n✅ DUMP COMPLETO SALVATO.");
}

exportEverythingFixed().catch(console.error);
