...
function classifyGre(title: string, sku: string): string | null {
    ...
    // NORDIC (Islanda/Greenland/Finlandia)
    if (t.includes("nordic") || t.includes("finlandia") || t.includes("islanda") || t.includes("greenland")) {
        if (s.includes("88") || t.includes("omega") || t.includes("islanda")) return "Islanda (Nordic Omega)";
        return "Greenland (Nordic Pali)";
    }
    // ANTRACITE (Skyathos/Kea/Anthracite/Iraklion)
    if (t.includes("antracite") || t.includes("grigio") || t.includes("kea") || t.includes("skyathos") || t.includes("iraklion")) {
        if (s.includes("88") || t.includes("omega") || t.includes("skyathos")) return "Skyathos (Antracite Omega)";
        return "Kea (Antracite Pali)";
    }
    // LEGNO (Mauritius/Amazonia/Pacific/Sicilia)
    if (t.includes("legno") || t.includes("mauritius") || t.includes("amazonia") || t.includes("pacific") || t.includes("sicilia")) {
        if (s.includes("88") || t.includes("omega") || t.includes("amazonia")) return "Amazonia (Legno Omega)";
        return "Mauritius (Legno Pali)";
    }
    // BIANCA (Atlantis/Fidji/Bora Bora/Haiti)
...
async function finalConsolidation() {
  console.log("🚀 Consolidamento Serie Gre (LOGICA SINONIMI)...");
  ...
  for (const series in groups) {
      ...
      for (const other of others) {
          for (const v of other.variants.nodes) {
              ...
              else if (other.title.includes("300")) dim = "Ø 300";
              else if (other.title.includes("240")) dim = "Ø 240";
      ...
      // 1. Rename Master Option
      const currentOption = master.options.find((o: any) => o.name === "Title");
      if (currentOption) {
          await shopifyRequest(`
          mutation productOptionsUpdate($productId: ID!, $options: [ProductOptionUpdateInput!]!) {
            productOptionsUpdate(productId: $productId, options: $options) { userErrors { message } }
          }
          `, {
            productId: master.id,
            options: [{ id: currentOption.id, name: "Misura" }]
          });
      }

      // 2. Update Master Title
      await shopifyRequest(`mutation productUpdate($input: ProductInput!) { productUpdate(input: $input) { product { id } } }`, {
          input: { id: master.id, title: `Piscina Gre Serie ${series}` }
      });
