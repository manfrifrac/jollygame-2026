...
async function smartTagging() {
  console.log("🏷️  Avvio Tagging Smart per Categorizzazione Automatica...");
  ...
  while (hasNextPage) {
    ...
    for (const product of res.data.products.nodes) {
      ...
      if (title.includes("pulitore") || title.includes("robot") || title.includes("aspiratore") || title.includes("spazzola") || title.includes("retino") || ["oa ", "ra ", "swy", "cnx", "voyager", "alpha", "sweepy", "vortex"].some(k => title.includes(k))) {
          ...
          } else if (title.includes("manuale") || title.includes("spazzola") || title.includes("retino") || title.includes("kit di pulizia")) {
      ...
      // 3. FILTRAGGIO
      if (title.includes("filtro") || title.includes("depuratore") || title.includes("monoblocco") || title.includes("sabbia") || title.includes("cartuccia filtrante") || title.includes("pompa") || title.includes("flopro") || title.includes("flo pro")) {
          if (!title.includes("calore") && !title.includes("inverter")) {
      ...
      // 5. RISCALDAMENTO
      if (title.includes("pompa di calore") || title.includes("riscaldamento") || ["z250", "z350", "z400", "z550", "z650", "inverter"].some(k => title.includes(k))) {
      ...
      if (newTags.size !== currentTags.length) {
          ...
          console.log(`✨ Aggiornamento TAG per: ${product.title}`);
          console.log(`   - Nuovi TAG: ${finalTags.filter(t => !currentTags.includes(t)).join(", ")}`);
  ...
  console.log(`\n\n✅ Tagging completato. Aggiornati ${totalUpdated} prodotti.`);
}

smartTagging().catch(console.error);
