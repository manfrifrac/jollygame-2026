...
async function applyDefinitiveConsolidationV4() {
  ...
  console.log("🚀 AVVIO CONSOLIDAMENTO DEFINITIVO SERIE GRE (FIX OPTION VALUES)...");
  ...
  for (const seriesFull in plan) {
      ...
      const uniqueDims = [...new Set(variants.map((v: any) => v.dim))];
      
      // 2. Prepare variants for productSet
      ...
      const res = await shopifyRequest(productSetMutation, {
          input: {
              ...
              productOptions: [{ 
                  name: "Misura", 
                  values: uniqueDims.map(d => ({ name: d })) 
              }],
...
applyDefinitiveConsolidationV4().catch(console.error);
