...
async function applyShapeConsolidation() {
  const plan = JSON.parse(fs.readFileSync("../../gre_family_import_plan_v2.json", "utf8"));
  ...
  console.log("🚀 AVVIO CONSOLIDAMENTO PER SERIE E FORMA...");

  // 1. PULIZIA TOTALE PISCINE GRE CONSOLIDATE PRECEDENTEMENTE
  console.log("🧹 Rimuovo prodotti consolidati precedenti per rifare tutto pulito...");
  ...
  while (hasNextPage) {
    ...
    for (const p of res.data.products.nodes) {
        if (p.title.includes("Piscina Gre Serie")) {
            toDelete.push(p.id);
        }
    }
  ...
  console.log(`🗑️  Rimossi ${toDelete.length} prodotti.`);

  // 2. CREAZIONE NUOVI PRODOTTI PER SERIE + FORMA
  ...
  for (const groupKey in plan) {
      ...
      `, {
          input: {
              ...
              productType: "Piscina",
              status: "ACTIVE",
              tags: ["Categoria:Piscine", "Listino:2026", `Shape:${groupKey.split(' ')[0]}`],
      ...
      if (res.data?.productSet?.product) {
          console.log(`   ✅ Prodotto impostato.`);
          if (bestImage) {
          ...
          console.error(`   ❌ Errore ${title}:`, res.data?.productSet?.userErrors);
      }
  }

  console.log("\n✅ CONSOLIDAMENTO PER SERIE E FORMA COMPLETATO!");
}

applyShapeConsolidation().catch(console.error);
