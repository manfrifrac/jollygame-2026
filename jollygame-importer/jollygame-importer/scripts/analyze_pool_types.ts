...
async function analyzePoolTypes() {
  console.log("🔍 Avvio analisi tecnica dettagliata tipologie piscine...");
  ...
  while (hasNextPage) {
    ...
    query getPools($cursor: String) {
      products(first: 250, after: $cursor, query: "product_type:Piscina OR title:Piscina") {
    ...
    pools.push(...res.data.products.nodes);
  ...
  // Aggregazione per capire i "cluster"
  const summary = {
  ...
  console.log("--- Per Materiale ---");
  console.table(summary.per_materiale);
  console.log("--- Per Forma ---");
...
analyzePoolTypes().catch(console.error);
